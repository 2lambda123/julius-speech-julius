...
void j_output_argument_help(FILE *fp) {
  ...
  fprintf(fp, "         htkparam/mfcfile  feature vectors in HTK parameter "
              "file format\n");
  fprintf(fp, "         outprob           outprob vectors in HTK parameter "
              "file format\n");
  fprintf(fp,
  ...
  fprintf(
      fp,
      "    [-NA host:unit]     get audio from NetAudio server at host:unit\n");
#endif
  fprintf(
      fp,
      "    [-adport portnum]   adinnet port number to listen         (%d)\n",
      jconf->input.adinnet_port);
  fprintf(fp, "    [-48]               enable 48kHz sampling with internal "
              "down sampler (OFF)\n");
  fprintf(
      fp,
  ...
  fprintf(
      fp,
      "    [-lvscale]          input level scaling factor (1.0: OFF) (%.1f)\n",
  ...
  fprintf(
      fp,
      "    [-powerthres value] rejection threshold of average power  (%.1f)\n",
  ...
  fprintf(
      fp,
      "    [-lv unsignedshort] input level threshold (0-32767)       (%d)\n",
      jconf->detect.level_thres);
  fprintf(
      fp,
      "    [-zc zerocrossnum]  zerocross num threshold per sec.      (%d)\n",
      jconf->detect.zero_cross_num);
  fprintf(
      fp,
      "    [-headmargin msec]  header margin length in msec.         (%d)\n",
      jconf->detect.head_margin_msec);
  fprintf(
      fp,
      "    [-tailmargin msec]  tail margin length in msec.           (%d)\n",
      jconf->detect.tail_margin_msec);
  fprintf(
      fp,
      "    [-chunksize sample] unit length for processing            (%d)\n",
  ...
  fprintf(fp, "    [-agc][-noagc]        enable/disable additional AGC on "
              "WebRTC VAD\n");
  fprintf(fp,
  ...
  fprintf(
      fp,
      "    -gmmnum num         GMM Gaussian pruning num              (%d)\n",
      jconf->reject.gmm_gprune_num);
  fprintf(fp, "    -gmmreject string   comma-separated list of noise model "
              "name to reject\n");
#ifdef GMM_VAD
  fprintf(fp, "\n GMM-based VAD:\n");
  fprintf(
      fp,
      "    -gmmmargin frames   backstep margin on speech trigger     (%d)\n",
      jconf->detect.gmm_margin);
  fprintf(
      fp,
      "    -gmmup score        up-trigger threshold                  (%.1f)\n",
      jconf->detect.gmm_uptrigger_thres);
  fprintf(
      fp,
      "    -gmmdown score      down-trigger threshold                (%.1f)\n",
  ...
  fprintf(
      fp,
      "    [-escale]           scaling log energy for enormal        (%.1f)\n",
      jconf->am_root->analysis.para_default.escale);
  fprintf(
      fp,
      "    [-silfloor]         energy silence floor in dB            (%.1f)\n",
      jconf->am_root->analysis.para_default.silFloor);
  fprintf(
      fp,
      "    [-delwin frame]     delta windows length (frame)          (%d)\n",
      jconf->am_root->analysis.para_default.delWin);
  fprintf(
      fp,
      "    [-accwin frame]     accel windows length (frame)          (%d)\n",
      jconf->am_root->analysis.para_default.accWin);
  fprintf(
      fp,
      "    [-hifreq freq]      freq. of upper band limit, off if <0  (%d)\n",
      jconf->am_root->analysis.para_default.hipass);
  fprintf(
      fp,
      "    [-lofreq freq]      freq. of lower band limit, off if <0  (%d)\n",
  ...
  fprintf(
      fp,
      "    [-sscalclen msec]   length of head silence for SS (msec)  (%d)\n",
      jconf->am_root->frontend.sscalc_len);
  fprintf(fp, "    [-ssload filename]  load constant noise spectrum from file "
              "for SS\n");
  fprintf(
      fp,
      "    [-ssalpha value]    alpha coef. for SS                    (%f)\n",
      jconf->am_root->frontend.ss_alpha);
  fprintf(
      fp,
      "    [-ssfloor value]    spectral floor for SS                 (%f)\n",
  ...
  fprintf(fp, "    [-cmnload file]     load initial CMN/CVN param from file on "
              "startup\n");
  fprintf(
      fp,
      "    [-cmnsave file]     save CMN/CVN param to file after each input\n");
  fprintf(fp, "    [-cmnstatic]        no MAP, use static CMN/CVN (use with "
              "-cmnload)\n");
  fprintf(fp,
          "    [-cvnstatic]        use static CVN only (use with -cmnload)\n");
  fprintf(fp, "    [-cmnnoupdate]      not update initial param while recog. "
              "(use with -cmnload)\n");
  fprintf(fp,
          "    [-cmnmapweight]     weight value of initial cm for MAP-CMN "
          "(%6.2f)\n",
          jconf->am_root->analysis.cmn_map_weight);
  fprintf(
      fp,
      "    [-cvn]              cepstral variance normalisation       (%s)\n",
      jconf->amnow->analysis.para.cvn ? "on" : "off");
  fprintf(
      fp,
      "    [-vtln alpha lowcut hicut] enable VTLN (1.0 to disable)   (%f)\n",
  ...
  fprintf(
      fp,
      "             best N     use N best score (default of n-gram, N=%d)\n",
  ...
  fprintf(fp,
          "    [-spmodel HMMname]  name of short pause model             "
          "(\"%s\")\n",
  ...
  fprintf(
      fp,
      "             heuristic     heuristic pruning (default for TM/PTM)\n");
#else
...
#endif
  fprintf(fp,
          "    [-tmix gaussnum]    Gaussian num threshold per mixture for "
          "pruning (%d)\n",
          jconf->am_root->mixnum_thres);
  fprintf(fp, "    [-gshmm hmmdefs]    monophone hmmdefs for GS\n");
  fprintf(fp,
          "    [-gsnum N]          N-best state will be selected        (%d)\n",
  ...
  fprintf(fp,
          "    [-lmp float float]  weight and penalty (tri: %.1f %.1f mono: "
          "%.1f %1.f)\n",
          DEFAULT_LM_WEIGHT_TRI_PASS1, DEFAULT_LM_PENALTY_TRI_PASS1,
          DEFAULT_LM_WEIGHT_MONO_PASS1, DEFAULT_LM_PENALTY_MONO_PASS1);
  fprintf(fp,
          "    [-lmp2 float float]       for 2nd pass (tri: %.1f %.1f mono: "
          "%.1f %1.f)\n",
          DEFAULT_LM_WEIGHT_TRI_PASS2, DEFAULT_LM_PENALTY_TRI_PASS2,
          DEFAULT_LM_WEIGHT_MONO_PASS2, DEFAULT_LM_PENALTY_MONO_PASS2);
  ...
  fprintf(
      fp,
      "    [-penalty1 float]   word insertion penalty (1st pass)     (%.1f)\n",
      jconf->search_root->lmp.penalty1);
  fprintf(
      fp,
      "    [-penalty2 float]   word insertion penalty (2nd pass)     (%.1f)\n",
  ...
  fprintf(
      fp,
      "    [-mapunk wordname]  (n-gram) map unknown words to this    (%s)\n",
      UNK_WORD_DEFAULT);
  fprintf(fp, "    [-forcedict]        ignore error entry and keep running\n");
  fprintf(fp, "    [-iwspword]         (n-gram) add short-pause word for "
              "inter-word CD sp\n");
  fprintf(
      fp,
      "    [-iwspentry entry]  (n-gram) word entry for \"-iwspword\" (%s)\n",
      IWSPENTRY_DEFAULT);
  ...
  fprintf(
      fp,
      "                          head - BOS silence model name       (%s)\n",
      jconf->lm_root->wordrecog_head_silence_model_name);
  fprintf(
      fp,
      "                          tail - EOS silence model name       (%s)\n",
      jconf->lm_root->wordrecog_tail_silence_model_name);
  fprintf(fp,
          "                           sp  - their name as context or \"NULL\" "
          "(%s)\n",
          (jconf->lm_root->wordrecog_silence_context_name[0] == '\0')
              ? "NULL"
              : jconf->lm_root->wordrecog_silence_context_name);
#ifdef DETERMINE
  fprintf(fp,
          "    -wed float int      thresholds for early word determination\n");
  ...
  fprintf(fp, "    [-b beamwidth]      beam width (by state num)             "
              "(guessed)\n");
  fprintf(fp, "                        (0: full search, -1: force guess)\n");
#ifdef SCORE_PRUNING
  fprintf(fp, "    [-bs score_width]   beam width (by score offset)          "
              "(disabled)\n");
  ...
  fprintf(
      fp,
      "    [-nlimit N]         keeps only N tokens on each state     (%d)\n",
      jconf->search_root->pass1.wpair_keep_nlimit);
#endif
#endif
#ifdef SEPARATE_BY_UNIGRAM
  fprintf(fp,
          "    [-sepnum wordnum]   (n-gram) # of hi-freq word isolated from "
          "tree (%d)\n",
  ...
  fprintf(
      fp,
      "    [-iwcache percent]  (n-gram) amount of inter-word LM cache (%3d)\n",
  ...
  fprintf(
      fp,
      "    [-b2 hyponum]       word envelope beam width (by hypo num) (%d)\n",
      jconf->search_root->pass2.enveloped_bestfirst_width);
  fprintf(
      fp,
      "    [-n N]              # of sentence to find                 (%d)\n",
      jconf->search_root->pass2.nbest);
  fprintf(
      fp,
      "    [-output N]         # of sentence to output               (%d)\n",
      jconf->search_root->output.output_hypo_maxnum);
#ifdef SCAN_BEAM
  fprintf(
      fp,
      "    [-sb score]         score beam threshold (by score)       (%.1f)\n",
      jconf->search_root->pass2.scan_beam_thres);
#endif
  fprintf(
      fp,
      "    [-s hyponum]        global stack size of hypotheses       (%d)\n",
      jconf->search_root->pass2.stack_size);
  fprintf(
      fp,
      "    [-m hyponum]        hypotheses overflow threshold num     (%d)\n",
      jconf->search_root->pass2.hypo_overflow);

  fprintf(
      fp,
      "    [-lookuprange N]    frame lookup range in word expansion  (%d)\n",
  ...
  fprintf(
      fp,
      "    [-iwsppenalty]      trans. penalty for iwsp (multipath)   (%.1f)\n",
  ...
  fprintf(
      fp,
      "    [-spdur]            length threshold of sp frames         (%d)\n",
      jconf->search_root->successive.sp_frame_duration);
#ifdef SPSEGMENT_NAIST
  fprintf(
      fp,
      "    [-spmargin]         backstep margin on speech trigger     (%d)\n",
      jconf->search_root->successive.sp_margin);
  fprintf(
      fp,
      "    [-spdelay]          delay on speech trigger               (%d)\n",
      jconf->search_root->successive.sp_delay);
#endif
  fprintf(fp, "    [-pausemodels str]  comma-delimited list of pause models "
              "for segment\n");
  ...
  fprintf(fp, "                        -1: not merge, leave same loc. with "
              "diff. score\n");
  fprintf(fp,
  ...
  fprintf(fp,
          "    [-graphcut num]     graph cut depth at postprocess (-1: "
          "disable)(%d)\n",
  ...
  fprintf(
      fp,
      "    [-graphboundloop num] max. num of boundary adjustment loop (%d)\n",
      jconf->search_root->graph.graphout_limit_boundary_loop_num);
#endif
#ifdef GRAPHOUT_SEARCH_DELAY_TERMINATION
  fprintf(fp, "    [-graphsearchdelay] inhibit search termination until 1st "
              "sent. found\n");
  ...
  fprintf(fp,
          "    [-mbr_weight float float] score and loss func. weight on MBR "
          "(%.1f %.1f)\n",
          jconf->search_root->mbr.score_weight,
          jconf->search_root->mbr.loss_weight);
#endif
  ...
  fprintf(
      fp,
      "    [-cmalpha f t s]    CM smoothing factor        (from, to, step)\n");
#else
  fprintf(
      fp,
      "    [-cmalpha value]    CM smoothing factor                    (%f)\n",
      jconf->search_root->annotate.cm_alpha);
#endif
#ifdef CM_SEARCH_LIMIT
  fprintf(
      fp,
      "    [-cmthres value]    CM threshold to cut hypo on 2nd pass   (%f)\n",
  ...
  fprintf(
      fp,
      "    [-proginterval]     interval of progout in msec           (%d)\n",
