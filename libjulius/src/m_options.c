...
static boolean check_section(Jconf *jconf, char *optname, short sec) {
  if (!jconf->optsectioning)
    return TRUE;

  if (jconf->optsection == sec)
    return TRUE;

  if (jconf->optsection == JCONF_OPT_DEFAULT)
    return TRUE;

  switch (sec) {
  case JCONF_OPT_GLOBAL:
    jlog("ERROR: \"%s\" is global option (should be before any instance "
         "declaration)",
         optname);
    break;
  case JCONF_OPT_AM:
    jlog("ERROR: \"%s\" is AM option", optname);
    break;
  case JCONF_OPT_LM:
    jlog("ERROR: \"%s\" is LM option", optname);
    break;
  case JCONF_OPT_SR:
    jlog("ERROR: \"%s\" is SR (search) option", optname);
    break;
  }
...
#endif
#define GET_TMPARG                                                             \
  if ((tmparg = next_arg(&i, argc, argv)) == NULL)                             \
  return FALSE
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (strmatch(argv[i], "-C")) { /* include jconf file  */
      ...
      if (tmparg[0] >= '0' && tmparg[0] <= '9') {
        jlog("ERROR: m_options: AM name \"%s\" not acceptable: first character "
      ...
      if (tmparg[0] >= '0' && tmparg[0] <= '9') {
        jlog("ERROR: m_options: LM name \"%s\" not acceptable: first character "
             "should not be a digit\n",
             tmparg);
        return FALSE;
      }
      ...
      GET_TMPARG;
      if (tmparg[0] == '-') {
        jlog("ERROR: m_options: -SR needs three arguments: module name, AM "
             "name and LM name\n");
        return FALSE;
      }
      if (tmparg[0] >= '0' && tmparg[0] <= '9') {
        jlog("ERROR: m_options: SR name \"%s\" not acceptable: first character "
             "should not be a digit\n",
             tmparg);
        return FALSE;
      }
      ...
      if (tmparg[0] >= '0' && tmparg[0] <= '9') { /* arg is number */
        if ((amconf = j_get_amconf_by_id(jconf, atoi(tmparg))) == NULL)
          return FALSE;
      } else { /* name string */
        if ((amconf = j_get_amconf_by_name(jconf, tmparg)) == NULL)
          return FALSE;
      }
      GET_TMPARG;
      if (tmparg[0] == '-') {
      ...
      if (tmparg[0] >= '0' && tmparg[0] <= '9') { /* arg is number */
        if ((lmconf = j_get_lmconf_by_id(jconf, atoi(tmparg))) == NULL)
          return FALSE;
      } else { /* name string */
        if ((lmconf = j_get_lmconf_by_name(jconf, tmparg)) == NULL)
          return FALSE;
      }
      ...
      continue;
    } else if (strmatch(argv[i],
                        "-nosectioncheck")) { /* disable section check */
      jconf->optsectioning = FALSE;
      continue;
    } else if (strmatch(argv[i], "-input")) { /* speech input */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL))
        return FALSE;
      GET_TMPARG;
      ...
      if (strmatch(tmparg, "file") || strmatch(tmparg, "rawfile")) {
      ...
      } else if (strmatch(tmparg, "htkparam") || strmatch(tmparg, "mfcfile") ||
                 strmatch(tmparg, "mfc")) {
        ...
        jconf->decodeopt.realtime_flag = TRUE;
#else
        jlog("ERROR: m_options: \"-input alsa\": ALSA support is not "
             "built-in\n");
        return FALSE;
#endif
        ...
        jconf->decodeopt.realtime_flag = TRUE;
#else
        jlog("ERROR: m_options: \"-input esd\": ESounD support is not "
             "built-in\n");
        return FALSE;
#endif
        ...
        jconf->decodeopt.realtime_flag = TRUE;
#else
        jlog("ERROR: m_options: \"-input pulseaudio\": PulseAudio support is "
             "not built-in\n");
        return FALSE;
#endif
#endif
        ...
        jconf->decodeopt.realtime_flag = FALSE;
#ifdef ENABLE_PLUGIN
      } else if ((sid = plugin_find_optname("adin_get_optname", tmparg)) !=
                 -1) { /* adin plugin */
      ...
      } else if ((sid = plugin_find_optname("fvin_get_optname", tmparg)) !=
                 -1) { /* vector input plugin */
        jconf->input.plugin_source = sid;
        jconf->input.type = INPUT_VECTOR;
        jconf->input.speech_input = SP_MFCMODULE;
        jconf->decodeopt.realtime_flag = FALSE;
      ...
      // strcpy((char*)mymalloc(strlen(tmparg)+1),tmparg);
      ...
      if (strmatch(tmparg, "on")) {
        ...
        jlog("ERROR: m_options: \"-force_realtime\" should be either \"on\" or "
             "\"off\"\n");
        return FALSE;
      }
      ...
      continue;
                        ...
                        "-realtime")) { /* equal to "-force_realtime on" */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL))
        return FALSE;
      jconf->decodeopt.forced_realtime = TRUE;
      jconf->decodeopt.force_realtime_flag = TRUE;
      continue;
                        ...
                        "-norealtime")) { /* equal to "-force_realtime off" */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL))
        return FALSE;
      jconf->decodeopt.forced_realtime = FALSE;
      jconf->decodeopt.force_realtime_flag = TRUE;
...
#else
      jlog("WARNING: m_options: WPAIR_KEEP_NLIMIT disabled, \"-nlimit\" "
           "ignored\n");
#endif
      continue;
      ...
      continue;
                        ...
                        "-graphrange")) { /* neighbor merge range frame */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                        ...
                        "-graphboundloop")) { /* neighbor merge range frame */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                        ...
                        "-graphsearchdelay")) { /* not do graph search
                                                   termination before the 1st
                                                   sentence is found */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      ...
      continue;
                        ...
                        "-nographsearchdelay")) { /* not do graph search
                                                     termination before the 1st
                                                     sentence is found */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      ...
      continue;
    } else if (strmatch(argv[i], "-multigramout")) { /* enable per-grammar
                                                        decoding on 2nd pass */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      jconf->searchnow->output.multigramout_flag = TRUE;
      continue;
    } else if (strmatch(argv[i],
                        "-nomultigramout")) { /* disable per-grammar decoding on
                                                 2nd pass */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      ...
      continue;
                        ...
                        "-sb")) { /* score envelope width in 2nd pass */
#ifdef SCAN_BEAM
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                   ...
                   argv[i],
                   "-cutsilence")) { /* force (long) silence detection on */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL))
        return FALSE;
      ...
      continue;
                   ...
                   argv[i],
                   "-nocutsilence")) { /* force (long) silence detection off */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL))
        return FALSE;
      jconf->detect.silence_cut = 0;
      continue;
    } else if (strmatch(argv[i],
                        "-pausesegment")) { /* force (long) silence detection on
      ...
      continue;
                   ...
                   argv[i],
                   "-nopausesegment")) { /* force (long) silence detection off
      ...
      continue;
                        ...
                        "-lv")) { /* silence detection threshold level */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                        ...
                        "-zc")) { /* silence detection zero cross num */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
    } else if (strmatch(argv[i], "-hipass") ||
               strmatch(argv[i],
                        "-hifreq")) { /* frequency of upper band limit */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      GET_TMPARG;
      jconf->amnow->analysis.para.hipass = atoi(tmparg);
      continue;
    } else if (strmatch(argv[i], "-lopass") ||
               strmatch(argv[i],
                        "-lofreq")) { /* frequency of lower band limit */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      GET_TMPARG;
      ...
      jconf->amnow->analysis.para.smp_freq =
          period2freq(jconf->amnow->analysis.para.smp_period);
      continue;
      ...
      continue;
                        ...
                        "-48")) { /* use 48kHz input and down to 16kHz */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL))
        return FALSE;
      jconf->input.use_ds48to16 = TRUE;
      continue;
    } else if (strmatch(argv[i], "-version") ||
               strmatch(argv[i], "--version") ||
               strmatch(argv[i], "-setting") ||
               strmatch(argv[i], "--setting")) { /* print version and exit */
      j_put_header(stderr);
      ...
      continue;
                        ...
                        "-callbackdebug")) { /* output callback debug message */
      ...
      continue;
                        ...
                        "-palign")) { /* do forced alignment by phoneme */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      jconf->searchnow->annotate.align_result_phoneme_flag = TRUE;
      continue;
    } else if (strmatch(argv[i],
                        "-salign")) { /* do forced alignment by state */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      ...
      continue;
                        ...
                        "-transp")) { /* penalty for transparent word */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                        ...
                        "-gram")) { /* comma-separatedlist of grammar prefix */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM))
        return FALSE;
      GET_TMPARG;
      if (multigram_add_prefix_list(tmparg, cwd, jconf->lmnow,
                                    LM_DFA_GRAMMAR) == FALSE) {
        jlog("ERROR: m_options: failed to read some grammars\n");
        return FALSE;
      }
      continue;
    } else if (strmatch(argv[i],
                        "-gramlist")) { /* file of grammar prefix list */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM))
        return FALSE;
      GET_TMPARG;
      tmparg = filepath(tmparg, cwd);
      if (multigram_add_prefix_filelist(tmparg, jconf->lmnow, LM_DFA_GRAMMAR) ==
          FALSE) {
        jlog("ERROR: m_options: failed to read some grammars\n");
        free(tmparg);
        return FALSE;
      ...
      continue;
                        ...
                        "-penalty1")) { /* word insertion penalty (pass1) */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                        ...
                        "-penalty2")) { /* word insertion penalty (pass2) */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
      jconf->searchnow->lmp.penalty2 = (LOGPROB)atof(tmparg);
      continue;
    } else if (strmatch(argv[i], "-spmodel") ||
               strmatch(argv[i], "-sp")) { /* name of short pause word */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      FREE_MEMORY(jconf->amnow->spmodel_name);
      GET_TMPARG;
      jconf->amnow->spmodel_name =
          strcpy((char *)mymalloc(strlen(tmparg) + 1), tmparg);
      continue;
      ...
      continue;
    } else if (strmatch(argv[i], "-iwsp")) { /* enable inter-word short pause
                                                handing (for multipath) */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM))
        return FALSE;
      ...
      continue;
                   ...
                   argv[i],
                   "-iwsppenalty")) { /* set inter-word short pause transition
                                         penalty (for multipath) */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      GET_TMPARG;
      ...
      GET_TMPARG;
      jconf->lmnow->iwspentry =
          strcpy((char *)mymalloc(strlen(tmparg) + 1), tmparg);
      continue;
    } else if (strmatch(argv[i],
                        "-iwcache")) { /* control cross-word LM cache */
#ifdef HASH_CACHE_IW
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
      jconf->searchnow->pass1.iw_cache_rate = atof(tmparg);
      if (jconf->searchnow->pass1.iw_cache_rate > 100)
        jconf->searchnow->pass1.iw_cache_rate = 100;
      if (jconf->searchnow->pass1.iw_cache_rate < 1)
        jconf->searchnow->pass1.iw_cache_rate = 1;
#else
      jlog(
          "WARNING: m_options: HASH_CACHE_IW disabled, \"-iwcache\" ignored\n");
#endif
      continue;
    } else if (strmatch(argv[i], "-sepnum")) { /* N-best frequent word will be
                                                  separated from tree */
...
#else
      jlog("WARNING: m_options: SEPARATE_BY_UNIGRAM disabled, \"-sepnum\" "
           "ignored\n");
      i++;
#endif
      continue;
      ...
      continue;
                        ...
                        "-zmean")) { /* enable DC offset by zero mean */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL))
        return FALSE;
      jconf->preprocess.use_zmean = TRUE;
      continue;
    } else if (strmatch(argv[i],
                        "-nozmean")) { /* disable DC offset by zero mean */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL))
        return FALSE;
      ...
      continue;
    } else if (strmatch(argv[i], "-nozmeanframe")) { /* disable frame-wise DC
                                                        offset by zero mean */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      ...
      continue;
                        ...
                        "-usepower")) { /* use power instead of magnitude in
                                           filterbank analysis */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      jconf->amnow->analysis.para.usepower = TRUE;
      continue;
    } else if (strmatch(argv[i], "-nousepower")) { /* use magnitude in fbank
                                                      analysis (default)  */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      ...
      continue;
    } else if (strmatch(argv[i], "-spdur")) { /* speech down-trigger duration
                                                 threshold in frame */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
    ...
    } else if (strmatch(argv[i], "-spmargin")) { /* speech up-trigger backstep
                                                    margin in frame */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                        ...
                        "-spdelay")) { /* speech up-trigger delay frame */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                        ...
                        "-pausemodels")) { /* short-pause duration threshold */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      FREE_MEMORY(jconf->searchnow->successive.pausemodelname);
      GET_TMPARG;
      jconf->searchnow->successive.pausemodelname =
          strcpy((char *)mymalloc(strlen(tmparg) + 1), tmparg);
      continue;
    } else if (strmatch(argv[i],
                        "-gprune")) { /* select Gaussian pruning method */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      GET_TMPARG;
      ...
      } else if (strmatch(tmparg,
                          "none")) { /* no prune: compute all Gaussian */
        jconf->amnow->gprune_method = GPRUNE_SEL_NONE;
      } else if (strmatch(tmparg, "default")) {
        jconf->amnow->gprune_method = GPRUNE_SEL_UNDEF;
#ifdef ENABLE_PLUGIN
      } else if ((sid = plugin_find_optname("calcmix_get_optname", tmparg)) !=
                 -1) { /* mixture calculation plugin */
       ...
       */
                        ...
                        "-no_ccd")) { /* force triphone handling = OFF */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      ...
      continue;
                        ...
                        "-force_ccd")) { /* force triphone handling = ON */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      jconf->searchnow->ccd_handling = TRUE;
      jconf->searchnow->force_ccd_handling = TRUE;
      continue;
    } else if (strmatch(argv[i], "-iwcd1")) { /* select cross-word triphone
                                                 computation method */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      GET_TMPARG;
      if (strmatch(tmparg,
                   "max")) { /* use maximum score in triphone variants */
      ...
      continue;
    } else if (strmatch(argv[i], "-b2") || strmatch(argv[i], "-bw") ||
               strmatch(argv[i], "-wb")) { /* word beam width in 2nd pass */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                        ...
                        "-booknum")) { /* num of state to select in GS */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                        ...
                        "-cmnload")) { /* load CMN parameter from file */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      ...
      continue;
                        ...
                        "-cmnnoupdate")) { /* not update CMN parameter */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      jconf->amnow->analysis.cmn_update = FALSE;
      continue;
    } else if (strmatch(argv[i],
                        "-cmnstatic")) { /* no map, just use static parameter */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      ...
      continue;
                        ...
                        "-cvnstatic")) { /* static variance but left mean
                                            dynamic on buffered input  */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      ...
      continue;
    } else if (strmatch(argv[i], "-sscalc")) { /* do spectral subtraction (SS)
                                                  for raw file input */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      ...
      continue;
    } else if (strmatch(argv[i], "-sscalclen")) { /* head silence length used to
                                                     compute SS (in msec) */
      if (!check_section(jconf, argv[i], JCONF_OPT_AM))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                        ...
                        "-cmalpha")) { /* CM log score scaling factor */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      ...
      jconf->searchnow->annotate.cm_alpha_num =
          (int)((jconf->searchnow->annotate.cm_alpha_end -
                 jconf->searchnow->annotate.cm_alpha_bgn) /
                jconf->searchnow->annotate.cm_alpha_step) +
          1;
      ...
      continue;
                        ...
                        "-cmthres")) { /* CM cut threshold for CM decoding */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                        ...
                        "-cmthres2")) { /* CM cut threshold for CM decoding */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      GET_TMPARG;
      ...
      continue;
                        ...
                        "-gmmnum")) { /* num of Gaussian pruning for GMM */
      if (!check_section(jconf, argv[i], JCONF_OPT_GLOBAL))
        return FALSE;
      GET_TMPARG;
      ...
      GET_TMPARG;
      tmparg = filepath(tmparg, cwd);
      if (htk_config_file_parse(tmparg, &(jconf->amnow->analysis.para_htk)) ==
          FALSE) {
      ...
      GET_TMPARG;
      tmparg = filepath(tmparg, cwd);
      if (multigram_add_prefix_filelist(tmparg, jconf->lmnow, LM_DFA_WORD) ==
          FALSE) {
      ...
      /*
       * if (jconf->lmnow->lmvar != LM_UNDEF && jconf->lmnow->lmvar !=
       * LM_DFA_WORD) { jlog("ERROR: \"-wsil\" only valid for isolated word
       * recognition mode\n"); return FALSE;
       * }
       */
      if (!check_section(jconf, argv[i], JCONF_OPT_LM))
        return FALSE;
      GET_TMPARG;
      strncpy(jconf->lmnow->wordrecog_head_silence_model_name, tmparg,
              MAX_HMMNAME_LEN);
      GET_TMPARG;
      strncpy(jconf->lmnow->wordrecog_tail_silence_model_name, tmparg,
              MAX_HMMNAME_LEN);
      GET_TMPARG;
      if (strmatch(tmparg, "NULL")) {
        jconf->lmnow->wordrecog_silence_context_name[0] = '\0';
      } else {
        strncpy(jconf->lmnow->wordrecog_silence_context_name, tmparg,
                MAX_HMMNAME_LEN);
      }
      continue;
#ifdef DETERMINE
      ...
      // if (jconf->lmnow->lmvar != LM_UNDEF && jconf->lmnow->lmvar !=
      // LM_DFA_WORD) { jlog("ERROR: \"-wed\" only valid for isolated word
      // recognition mode\n"); return FALSE;
      // }
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
      ...
      continue;
    } else if (strmatch(argv[i], "-fallback1pass")) { /* use 1st pass result on
                                                         search failure */
      if (!check_section(jconf, argv[i], JCONF_OPT_SR))
        return FALSE;
    ...
    if (argv[i][0] == '-' && strlen(argv[i]) == 2) {
      ...
      case 'w': /* word list (isolated word recognition) */
        if (!check_section(jconf, argv[i], JCONF_OPT_LM))
          return FALSE;
        GET_TMPARG;
        if (multigram_add_prefix_list(tmparg, cwd, jconf->lmnow, LM_DFA_WORD) ==
            FALSE) {
  ...
  for (atmp = jconf->am_root; atmp; atmp = atmp->next) {
    if (!atmp->spmodel_name) {
      atmp->spmodel_name =
          strcpy((char *)mymalloc(strlen(SPMODEL_NAME_DEFAULT) + 1),
                 SPMODEL_NAME_DEFAULT);
    }
  }
  for (ltmp = jconf->lm_root; ltmp; ltmp = ltmp->next) {
    if (!ltmp->head_silname) {
      ltmp->head_silname = strcpy(
          (char *)mymalloc(strlen(BEGIN_WORD_DEFAULT) + 1), BEGIN_WORD_DEFAULT);
    }
    if (!ltmp->tail_silname) {
      ltmp->tail_silname = strcpy(
          (char *)mymalloc(strlen(END_WORD_DEFAULT) + 1), END_WORD_DEFAULT);
    }
    if (!ltmp->iwspentry) {
  ...
  if (!jconf->input.netaudio_devname) {
    jconf->input.netaudio_devname = strcpy(
        (char *)mymalloc(strlen(NETAUDIO_DEVNAME) + 1), NETAUDIO_DEVNAME);
  }
#endif /* USE_NETAUDIO */
