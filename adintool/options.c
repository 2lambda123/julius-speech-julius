...
boolean show_help_and_exit(Jconf *jconf, char *arg[], int argnum) {
  ...
  fprintf(stderr,
          "  (other input can be specified by \"-input xxx\" as in Julius)\n");
  fprintf(stderr, "outputdev: output data to:\n");
  fprintf(stderr,
          "    file        speech file (\"foo.0000.wav\" - \"foo.N.wav\"\n");
  fprintf(stderr, "    adinnet     to adinnet server (I'm client)\n");
  fprintf(stderr,
          "    vecnet      to vecnet server as feature vector (I'm client)\n");
...
#endif
  fprintf(stderr,
          "    -server host[,host,...] (adinnet-out) server hostnames\n");
  fprintf(stderr,
          "    -port num[,num,...]     (adinnet-out) port numbers (%d)\n",
          ADINNET_PORT);
  ...
  fprintf(stderr,
          " (input segmentation: on for file/mic/stdin, off for adinnet)\n");
  fprintf(stderr, "  [-nosegment]          not segment input speech\n");
  fprintf(stderr,
          "  [-segment]            force segmentation of input speech\n");
  ...
  fprintf(stderr,
          ...
          "rejects noises aggressively, -1 to disable) (%d)\n",
  ...
  fprintf(
      stderr,
      "  [-agc][-noagc]        enable/disable additional AGC on WebRTC VAD\n");
  fprintf(stderr,
          "  [-agc_param p1 ... p7]  AGC parameters   (%d %.2f %.2f %.2f %.2f "
          "%.2f %.2f)\n",
  ...
  fprintf(stderr,
          "  [-autopause]          automatically pause at each input end\n");
  fprintf(stderr,
          "  [-loosesync]          loose sync of resume among servers\n");
  fprintf(
      stderr,
      "  [-rewind msec]        rewind input if spoken while pause at resume\n");
  fprintf(stderr, "  [-C jconffile]        load jconf to set parameters "
...
static boolean opt_server(Jconf *jconf, char *arg[], int argnum) {
  char *p, *q;
  if (global_a->conf.speech_output == SPOUT_ADINNET ||
      global_a->conf.speech_output == SPOUT_VECTORNET) {
    ...
    for (q = strtok(p, ","); q; q = strtok(NULL, ",")) {
      ...
      global_a->conf.adinnet_serv[global_a->conf.adinnet_servnum] =
          (char *)malloc(strlen(q) + 1);
    ...
    fprintf(stderr,
            "Warning: server [%s] should be used with adinnet / vecnet\n",
            arg[0]);
    return FALSE;
  }
  return TRUE;
...
static boolean opt_paramtype(Jconf *jconf, char *arg[], int argnum) {
...
static boolean opt_veclen(Jconf *jconf, char *arg[], int argnum) {
...
void register_options_to_julius() {
  ...
  j_add_option("-filename", 1, 1, "(base) filename to record (-out file)",
               opt_filename);
  j_add_option("-paramtype", 1, 1, "feature parameter type in HTK format",
               opt_paramtype);
  ...
  j_add_option("-nosegment", 0, 0, "not segment input speech, record all",
               opt_nosegment);
  ...
  j_add_option("-autopause", 0, 0, "automatically pause at each input end",
               opt_autopause);
  j_add_option("-loosesync", 0, 0, "loose sync of resume among servers",
               opt_loosesync);
