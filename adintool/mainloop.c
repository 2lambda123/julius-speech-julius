...
static int process_callback_file(SP16 *now, int len, Recog *recog) {
  AdinTool *a = global_a;
  int count;
  int start;
  int w;
...
static int process_callback_adinnet(SP16 *now, int len, Recog *recog) {
  AdinTool *a = global_a;
  int count;
  int start, w;
  int i;
  ...
  for (i = 0; i < a->conf.adinnet_servnum; i++) {
    ...
    if (count < 0) {
      ...
      fprintf(stderr, "failed to send data to %s:%d\n", a->conf.adinnet_serv[i],
              a->conf.adinnet_port[i]);
    }
  }
#ifdef WORDS_BIGENDIAN
  swap_sample_bytes(&(now[start]), len);
...
boolean vecnet_init(Recog *recog) {
  ...
  calc_para_from_header(&(jconf->am_root->analysis.para),
                        a->conf.vecnet_paramtype, a->conf.vecnet_veclen);
  ...
  if (recog->jconf->input.type == INPUT_WAVEFORM) {
    for (mfcc = recog->mfcclist; mfcc; mfcc = mfcc->next) {
      if (mfcc->frontend.sscalc) {
        ...
        if (mfcc->frontend.sscalc_len * recog->jconf->input.sfreq / 1000 <
            mfcc->para->framesize) {
          return FALSE;
        }
      }
  ...
  if (recog->jconf->input.type == INPUT_WAVEFORM) {
    if (RealTimeInit(recog) == FALSE) {
      fprintf(stderr,
              "Error: failed to initialize feature extraction module\n");
      return FALSE;
    }
  }
...
static int vecnet_send_data(int sd, void *buf, int bytes) {
...
static void vecnet_send_header(Recog *recog) {
  AdinTool *a = global_a;
  ConfigurationHeader conf;
  int i;

  conf.veclen = recog->jconf->am_root->analysis.para.veclen;
  conf.fshift = 1000.0 * recog->jconf->am_root->analysis.para.frameshift /
                recog->jconf->am_root->analysis.para.smp_freq;
...
static boolean vecnet_prepare(Recog *recog) {
  AdinTool *a = global_a;
...
static void vecnet_sub(SP16 *Speech, int nowlen, Recog *recog) {
  AdinTool *a = global_a;
  int i, j, now;
  MFCCCalc *mfcc;
  RealBeam *r = &(recog->real);
  ...
  while (now < nowlen) {
    ...
    for (mfcc = recog->mfcclist; mfcc; mfcc = mfcc->next) {
      ...
      if (RealTimeMFCC(mfcc, r->window, r->windowlen)) {
        ...
        memcpy(mfcc->param->parvec[mfcc->f], mfcc->tmpmfcc,
               sizeof(VECT) * mfcc->param->veclen);
#if 0
                {
                    int i;
        ...
        for (j = 0; j < a->conf.adinnet_servnum; j++) {
          vecnet_send_data(a->sd[j], mfcc->tmpmfcc,
                           sizeof(VECT) * a->conf.vecnet_veclen);
        }
        mfcc->f++;
      }
    }
    /* shift window */
    memmove(r->window, &(r->window[recog->jconf->input.frameshift]),
            sizeof(SP16) * (r->windowlen - recog->jconf->input.frameshift));
    r->windownum -= recog->jconf->input.frameshift;
  }
}

// finish feature calculation at end of audio segment
static void vecnet_param_update(Recog *recog) {
  MFCCCalc *mfcc;
...
static int process_callback_vecnet(SP16 *now, int len, Recog *recog) {
  AdinTool *a = global_a;
  int start, w;
...
static void vecnet_send_end_of_segment() {
  AdinTool *a = global_a;
  int i, j;
...
static void vecnet_send_end_of_session() {
  AdinTool *a = global_a;
  int i, j;
 ...
 * </EN>
 */
static void adin_send_end_of_segment() {
  AdinTool *a = global_a;
  char p;
  int i;
 ...
 */
static int adinnet_check_command() {
  AdinTool *a = global_a;
  fd_set rfds;
  struct timeval tv;
  int status;
      ...
      if (FD_ISSET(a->sd[i], &rfds)) {
        ...
        default:
          ...
          if (a->unknown_command_counter > 100) {
            fprintf(stderr,
                    "killed by a flood of unknown commands from server\n");
            exit(1);
          }
        }
      }
...
#else
    status =
        select(max_sd + 1, &rfds, NULL, NULL, NULL); /* block when no command */
#endif
    if (status < 0) { /* error */
      fprintf(stderr, "adintool: cannot check command from adinnet server\n");
      a->process_error = TRUE;
...
static boolean close_files() {
  ...
  if (a->writing_file) {
    ...
    printf("%s: %d samples (%.2f sec.) [%6d (%5.2fs) - %6d (%5.2fs)]\n",
           a->outpath, a->speechlen, (float)a->speechlen / (float)a->conf.sfreq,
           a->trigger_sample, (float)a->trigger_sample / (float)a->conf.sfreq,
           a->trigger_sample + a->speechlen,
           (float)(a->trigger_sample + a->speechlen) / (float)a->conf.sfreq);
...
static int connect_to_output_device() {
  ...
  if (a->conf.speech_output == SPOUT_ADINNET ||
      a->conf.speech_output == SPOUT_VECTORNET) {
    /* connect to adinnet server(s) */
    for (i = 0; i < a->conf.adinnet_servnum; i++) {
      fprintf(stderr, "connecting to #%d (%s:%d)...", i + 1,
              a->conf.adinnet_serv[i], a->conf.adinnet_port[i]);
      a->sd[i] =
          make_connection(a->conf.adinnet_serv[i], a->conf.adinnet_port[i]);
      if (a->sd[i] < 0)
        return -1; /* on error */
    }
...
// close processing
static void close_processing(Recog *recog) {
  AdinTool *a = global_a;
  int i;
...
static void draw_wave(Recog *recog, SP16 *now, int len, void *data) {
  ...
  if (s->window == NULL) {
    ...
    s->window = SDL_CreateWindow("adintool", SDL_WINDOWPOS_UNDEFINED,
  ...
  while (1) {
    ...
    if (i < s->ticklen) {
      ...
      if (s->is_valid_flag == 0 && recog->adin->is_valid_data == TRUE) {
        ...
        for (m = (recog->adin->zc.valid_len * 1000 / freq) /
                 WAVE_TICK_TIME_MSEC;
             m > 0; m--) {
          k = s->bp - m;
          if (k < 0)
...
#ifdef AUTO_ADJUST_THRESHOLD
      /* get mean and variance for maximum levels of last AUTORHES_WINDOW_SEC
       * seconds */
      {
        int i, j;
        float mean = 0.0;
        float var = 0.0;
        int windowlen =
            (int)(AUTOTHRES_WINDOW_SEC * 1000.0 / WAVE_TICK_TIME_MSEC);
        if (windowlen > s->items)
          windowlen = s->items;
        if (windowlen > s->totaltick)
          windowlen = s->totaltick;
      ...
      /* get long mean, long variance of variance, and trigger rate of last
       * AUTOTHRES_STABLE_SEC seconds */
      {
        int i, j, c;
        float mean = 0.0;
        float var = 0.0;
        ...
        int windowlen =
            (int)(AUTOTHRES_STABLE_SEC * 1000.0 / WAVE_TICK_TIME_MSEC);
        if (windowlen > s->items)
          windowlen = s->items;
        if (windowlen > s->totaltick)
          windowlen = s->totaltick;
  ...
  if (s->totaltick <
      (int)(AUTOTHRES_START_IGNORE_SEC * 1000.0 / WAVE_TICK_TIME_MSEC)) {
    ...
    /* get long mean, long variance of variance, and trigger rate of last
     * AUTOTHRES_STABLE_SEC seconds */
    if (s->totaltick < (int)((AUTOTHRES_START_IGNORE_SEC + 1.0) * 1000.0 /
                             WAVE_TICK_TIME_MSEC)) {
      ...
      recog->adin->thres =
          recog->adin->thres * (1.0 - AUTOTHRES_ADAPT_SPEED_COEF * 2.0) +
          thres * AUTOTHRES_ADAPT_SPEED_COEF * 2.0;
      recog->jconf->detect.level_thres = recog->adin->thres;
      recog->adin->zc.trigger = recog->adin->thres;
      thres_moving = TRUE;
    } else {
      if (s->varofmean[j] < s->vvthres1 && s->triggerrate[j] > 0.9) {
        ...
        if (thres < THRESHOLD_ADJUST_MIN)
          thres = THRESHOLD_ADJUST_MIN;
        recog->adin->thres =
            recog->adin->thres * (1.0 - AUTOTHRES_ADAPT_SPEED_COEF) +
            thres * AUTOTHRES_ADAPT_SPEED_COEF;
        recog->jconf->detect.level_thres = recog->adin->thres;
        recog->adin->zc.trigger = recog->adin->thres;
        thres_moving = TRUE;
  ...
  // draw threshold bar
  float y1 = (1.0 - (float)recog->jconf->detect.level_thres / 32768.0) *
             viewport.h * 0.5;
  float y2 = (1.0 + (float)recog->jconf->detect.level_thres / 32768.0) *
             viewport.h * 0.5;
  if (thres_moving) {
    r.x = 0;
    r.y = y1;
  ...
  for (i = 0; i < s->items; i++) {
    ...
    s->rects[i].w = WAVE_TICK_WIDTH;
    s->rects[i].h =
        (int)((1.0 - s->minlevel[j]) * viewport.h * 0.5) - s->rects[i].y;
  ...
  for (i = 0; i < s->items; i++) {
    if ((s->rectflags[i] & process_flag) != m) {
      ...
      if ((m & WAVE_TICK_FLAG_PROCESSED) == 0 &&
          (s->rectflags[i] & WAVE_TICK_FLAG_PROCESSED) != 0) {
        startx = i;
        miny = viewport.h;
      }
  ...
  if (s->window != NULL && s->renderer != NULL) {
    while (SDL_PollEvent(&event)) {
      ...
      case SDLK_UP:
        ...
        if (recog->adin->thres > THRESHOLD_ADJUST_MAX)
          recog->adin->thres = THRESHOLD_ADJUST_MAX;
        recog->jconf->detect.level_thres = recog->adin->thres;
        recog->adin->zc.trigger = recog->adin->thres;
        break;
      case SDLK_DOWN:
        ...
        if (event.key.state != SDL_PRESSED)
          break;
        recog->adin->thres -= THRESHOLD_ADJUST_STEP;
        if (recog->adin->thres < THRESHOLD_ADJUST_MIN)
          recog->adin->thres = THRESHOLD_ADJUST_MIN;
        recog->jconf->detect.level_thres = recog->adin->thres;
        recog->adin->zc.trigger = recog->adin->thres;
        break;
      case SDLK_m:
        ...
        if (recog->jconf->preprocess.level_coef != 0.0f) {
          ...
          recog->jconf->preprocess.level_coef = recog->adin->level_coef =
              stored_scale;
        }
        break;
      case SDLK_c:
...
void mainloop() {
  ...
  while (1) {
    ...
    do {
      ...
      performed and, process_callback_* is called for speech output for each
      segment block.
          */
      /* adin_go() return when input segmented by long silence, or input stream
       * reached to the end */
      a->speechlen = 0;
      ...
      if (a->conf.speech_output == SPOUT_FILE) {
        ...
        if (a->speechlen > 0) {
          ...
          if (ret >= 0 || a->stop_at_next) { /* segmented by adin-cut or end of
                                                stream or server-side command */
          ...
          printf("sent: %d samples (%.2f sec.) [%6d (%5.2fs) - %6d (%5.2fs)]\n",
                 ...
                 a->trigger_sample,
                 (float)a->trigger_sample / (float)a->conf.sfreq,
                 a->trigger_sample + a->speechlen,
                 (float)(a->trigger_sample + a->speechlen) /
                     (float)a->conf.sfreq);
        }
      } else if (a->conf.speech_output == SPOUT_VECTORNET) {
        if (a->speechlen > 0) {
...
record_end:
  ...
  if (a->conf.speech_output == SPOUT_FILE) {
    if (a->conf.continuous_segment) {
      printf("recorded total %d samples (%.2f sec.) segmented to %s.%04d - "
             "%s.%04d files\n",
             a->total_speechlen,
             (float)a->total_speechlen / (float)a->conf.sfreq, a->conf.filename,
             0, a->conf.filename, a->sid - 1);
    }
  }
